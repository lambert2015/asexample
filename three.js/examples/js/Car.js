...
THREE.Car = function () {
	...
	this.updateCarModel = function ( delta, controls ) {
		...
		if ( controls.moveForward ) {
			...
			this.speed = THREE.MathUtil.clamp( this.speed + delta * this.FRONT_ACCELERATION, this.MAX_REVERSE_SPEED, this.MAX_SPEED );
			this.acceleration = THREE.MathUtil.clamp( this.acceleration + delta, -1, 1 );
		...
		if ( controls.moveBackward ) {
			...
			this.speed = THREE.MathUtil.clamp( this.speed - delta * this.BACK_ACCELERATION, this.MAX_REVERSE_SPEED, this.MAX_SPEED );
			this.acceleration = THREE.MathUtil.clamp( this.acceleration - delta, -1, 1 );
		...
		if ( controls.moveLeft ) {
			...
			this.wheelOrientation = THREE.MathUtil.clamp( this.wheelOrientation + delta * this.WHEEL_ANGULAR_ACCELERATION, - this.MAX_WHEEL_ROTATION, this.MAX_WHEEL_ROTATION );
		...
		if ( ! ( controls.moveForward || controls.moveBackward ) ) {
			...
			if ( this.speed > 0 ) {
				...
				this.speed = THREE.MathUtil.clamp( this.speed - k * delta * this.FRONT_DECCELERATION, 0, this.MAX_SPEED );
				this.acceleration = THREE.MathUtil.clamp( this.acceleration - k * delta, 0, 1 );

			} else {

				var k = exponentialEaseOut( this.speed / this.MAX_REVERSE_SPEED );

				this.speed = THREE.MathUtil.clamp( this.speed + k * delta * this.BACK_ACCELERATION, this.MAX_REVERSE_SPEED, 0 );
				this.acceleration = THREE.MathUtil.clamp( this.acceleration + k * delta, -1, 0 );
		...
		if ( ! ( controls.moveLeft || controls.moveRight ) ) {
			...
			if ( this.wheelOrientation > 0 ) {
				...
				this.wheelOrientation = THREE.MathUtil.clamp( this.wheelOrientation - delta * this.WHEEL_ANGULAR_DECCELERATION, 0, this.MAX_WHEEL_ROTATION );

			} else {

				this.wheelOrientation = THREE.MathUtil.clamp( this.wheelOrientation + delta * this.WHEEL_ANGULAR_DECCELERATION, - this.MAX_WHEEL_ROTATION, 0 );
