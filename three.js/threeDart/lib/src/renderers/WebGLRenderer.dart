class WebGLRenderer implements Renderer {
  ...
  Element get domElement => canvas;
		...
		if ( material.attributes != null ) {
			...
			for ( var a in material.attributes ) {
				...
				if( !attribute["__webglInitialized"] || attribute.createUniqueBuffers ) {
					...
					if ( identical(attribute.type, "v2") ) { size = 2;
					} else if ( identical(attribute.type, "v3") ) { size = 3;
					} else if ( identical(attribute.type, "v4") ) { size = 4;
					} else if ( identical(attribute.type, "c")  ) size = 3;
		...
		if ( object.sortParticles ) {
			...
			if ( customAttributes != null ) {
				...
				for ( i = 0; i < il; i ++ ) {
					...
					if ( ! ( customAttribute.boundTo == null || identical(customAttribute.boundTo, "vertices") ) ) continue;
					...
					if ( customAttribute.size == 1 ) {
						...
						if ( identical(customAttribute.type, "c") ) {
	...
	setMeshBuffers( WebGLGeometry geometryGroup, WebGLObject object, hint, dispose, material ) {
		...
		needsSmoothNormals = ( identical(normalType, Three.SmoothShading) );
		...
		if ( dirtyColors && vertexColorType ) {
			...
			for ( f = 0; f < fl; f ++ ) {
				...
				if ( vertexColors.length == 3 && identical(vertexColorType, Three.VertexColors) ) {
			...
			for ( f = 0; f < fl; f ++ ) {
				...
				if ( vertexColors.length == 4 && identical(vertexColorType, Three.VertexColors) ) {
		...
		if ( customAttributes != null) {
			...
			for ( i = 0; i < il; i ++ ) {
				...
				if ( customAttribute.size == 1 ) {
					...
					if ( customAttribute.boundTo == null || identical(customAttribute.boundTo, "vertices") ) {
					...
					} else if ( identical(customAttribute.boundTo, "faces") ) {
					...
					} else if ( identical(customAttribute.boundTo, "faceVertices") ) {
	...
	renderBufferImmediate ( object, program, material ) {
		...
		if ( object.hasNormals ) {
			...
			if ( identical(material.shading, Three.FlatShading) ) {
		...
		if ( object.hasColors && !identical(material.vertexColors, Three.NoColors) ) {
	...
	renderBufferDirect ( camera, lights, fog, material, geometry, object ) {
		...
		if ( identical(material.visible, false) ) return;
		...
		if ( !identical(geometryHash, _currentGeometryGroupHash) ) {
	...
	renderBuffer ( camera, lights, fog, material, WebGLGeometry geometryGroup, WebGLObject webglobject ) {
	  ...
	  Object3D object = webglobject.object;
		...
		if ( !identical(geometryGroupHash, _currentGeometryGroupHash) ) {
	...
	_render ( Scene scene, Camera c, [renderTarget = null, forceClear = false] ) {
		...
		if ( (renderTarget != null ) && renderTarget.generateMipmaps && !identical(renderTarget.minFilter, Three.NearestFilter) && !identical(renderTarget.minFilter, Three.LinearFilter) ) {
	...
	renderObjects (  List<WebGLObject>renderList,
		...
		for ( var i = start; !identical(i, end); i += delta ) {
	...
	initMaterial( WebGLMaterial material, List<Light> lights, Fog fog, WebGLObject webglobject ) {
	  ...
	  Object3D object = webglobject.object;
		...
		material.program = buildProgram(
			...
			doubleSided: identical(material.side, Three.DoubleSide) );
	...
	setProgram( WebGLCamera camera, List lights, Fog fog, WebGLMaterial material, WebGLObject object ) {
		...
		if ( !identical(material.id, _currentMaterialId) ) {
	...
	loadUniformsGeneric ( program, uniforms ) {
		...
		for ( j = 0; j < jl; j ++ ) {
			...
			if ( identical(type, "i") ) { // single integer

				_gl.uniform1i( location, value );

			} else if ( identical(type, "f") ) { // single float

				_gl.uniform1f( location, value );

			} else if ( identical(type, "v2") ) { // single THREE.Vector2

				_gl.uniform2f( location, value.x, value.y );

			} else if ( identical(type, "v3") ) { // single THREE.Vector3

				_gl.uniform3f( location, value.x, value.y, value.z );

			} else if ( identical(type, "v4") ) { // single THREE.Vector4

				_gl.uniform4f( location, value.x, value.y, value.z, value.w );

			} else if ( identical(type, "c") ) { // single THREE.Color

				_gl.uniform3f( location, value.r, value.g, value.b );

			} else if ( identical(type, "iv1") ) { // flat array of integers (JS or typed array)

				_gl.uniform1iv( location, (value is List) ? new Float32Array.fromList(value) : value );

			} else if ( identical(type, "iv") ) { // flat array of integers with 3 x N size (JS or typed array)

				_gl.uniform3iv( location, (value is List) ? new Float32Array.fromList(value) : value );

			} else if ( identical(type, "fv1") ) { // flat array of floats (JS or typed array)

				_gl.uniform1fv( location, (value is List) ? new Float32Array.fromList(value) : value );

			} else if ( identical(type, "fv") ) { // flat array of floats with 3 x N size (JS or typed array)

				_gl.uniform3fv( location, (value is List) ? new Float32Array.fromList(value) : value );

			} else if ( identical(type, "v2v") ) { // array of THREE.Vector2
			...
			} else if ( identical(type, "v3v") ) { // array of THREE.Vector3
			...
			} else if ( identical(type, "v4v") ) { // array of THREE.Vector4
			...
			} else if ( identical(type, "m4")) { // single THREE.Matrix4
			...
			} else if ( identical(type, "m4v") ) { // array of THREE.Matrix4
			...
			} else if ( identical(type, "t") ) { // single THREE.Texture (2d or cube)
			...
			} else if ( identical(type, "tv") ) { // array of THREE.Texture (2d)
	...
	setFaceCulling( cullFace, frontFace ) {
		...
		if ( cullFace ) {
			...
			if ( !frontFace || identical(frontFace, "ccw") ) {
			...
			if( identical(cullFace, "back") ) {

				_gl.cullFace( WebGLRenderingContext.BACK );

			} else if( identical(cullFace, "front") ) {
	...
	setMaterialFaces( material ) {
		...
		var doubleSided = identical(material.side, Three.DoubleSide);
		var flipSided = identical(material.side, Three.BackSide);

		if ( !identical(_oldDoubleSided, doubleSided) ) {
		...
		if ( !identical(_oldFlipSided, flipSided) ) {
	...
	setDepthTest( depthTest ) {
		...
		if ( !identical(_oldDepthTest, depthTest) ) {
	...
	setDepthWrite( depthWrite ) {
		...
		if ( !identical(_oldDepthWrite, depthWrite) ) {
	...
	setLineWidth ( width ) {
		...
		if ( !identical(width, _oldLineWidth) ) {
	...
	setPolygonOffset ( polygonoffset, factor, units ) {
		...
		if ( !identical(_oldPolygonOffset, polygonoffset) ) {
		...
		if ( polygonoffset && ( !identical(_oldPolygonOffsetFactor, factor) || !identical(_oldPolygonOffsetUnits, units) ) ) {
	...
	setBlending( blending, [blendEquation, blendSrc, blendDst] ) {
		...
		if ( !identical(blending, _oldBlending) ) {

			if ( identical(blending, Three.NoBlending) ) {

				_gl.disable( WebGLRenderingContext.BLEND );

			} else if ( identical(blending, Three.AdditiveBlending) ) {
			...
			} else if ( identical(blending, Three.SubtractiveBlending) ) {
			...
			} else if ( identical(blending, Three.MultiplyBlending) ) {
			...
			} else if ( identical(blending, Three.CustomBlending) ) {
		...
		if ( identical(blending, Three.CustomBlending) ) {

			if ( !identical(blendEquation, _oldBlendEquation) ) {
			...
			if ( !identical(blendSrc, _oldBlendSrc) || !identical(blendDst, _oldBlendDst) ) {
	...
	setTextureParameters ( textureType, texture, isImagePowerOfTwo ) {
		...
		if ( (_glExtensionTextureFilterAnisotropic != null) && !identical(texture.type, Three.FloatType) ) {
	...
	filterFallback ( f ) {
		...
		if ( identical(f, Three.NearestFilter) || identical(f, Three.NearestMipMapNearestFilter) || identical(f, Three.NearestMipMapLinearFilter) ) {
	...
	paramThreeToGL ( p ) {
		...
		if ( identical(p, Three.RepeatWrapping) ) return WebGLRenderingContext.REPEAT;
		if ( identical(p, Three.ClampToEdgeWrapping) ) return WebGLRenderingContext.CLAMP_TO_EDGE;
		if ( identical(p, Three.MirroredRepeatWrapping) ) return WebGLRenderingContext.MIRRORED_REPEAT;

		if ( identical(p, Three.NearestFilter) ) return WebGLRenderingContext.NEAREST;
		if ( identical(p, Three.NearestMipMapNearestFilter) ) return WebGLRenderingContext.NEAREST_MIPMAP_NEAREST;
		if ( identical(p, Three.NearestMipMapLinearFilter) ) return WebGLRenderingContext.NEAREST_MIPMAP_LINEAR;

		if ( identical(p, Three.LinearFilter) ) return WebGLRenderingContext.LINEAR;
		if ( identical(p, Three.LinearMipMapNearestFilter) ) return WebGLRenderingContext.LINEAR_MIPMAP_NEAREST;
		if ( identical(p, Three.LinearMipMapLinearFilter) ) return WebGLRenderingContext.LINEAR_MIPMAP_LINEAR;

		if ( identical(p, Three.UnsignedByteType) ) return WebGLRenderingContext.UNSIGNED_BYTE;
		if ( identical(p, Three.UnsignedShort4444Type) ) return WebGLRenderingContext.UNSIGNED_SHORT_4_4_4_4;
		if ( identical(p, Three.UnsignedShort5551Type) ) return WebGLRenderingContext.UNSIGNED_SHORT_5_5_5_1;
		if ( identical(p, Three.UnsignedShort565Type) ) return WebGLRenderingContext.UNSIGNED_SHORT_5_6_5;

		if ( identical(p, Three.ByteType) ) return WebGLRenderingContext.BYTE;
		if ( identical(p, Three.ShortType) ) return WebGLRenderingContext.SHORT;
		if ( identical(p, Three.UnsignedShortType) ) return WebGLRenderingContext.UNSIGNED_SHORT;
		if ( identical(p, Three.IntType) ) return WebGLRenderingContext.INT;
		if ( identical(p, Three.UnsignedIntType) ) return WebGLRenderingContext.UNSIGNED_INT;
		if ( identical(p, Three.FloatType) ) return WebGLRenderingContext.FLOAT;

		if ( identical(p, Three.AlphaFormat) ) return WebGLRenderingContext.ALPHA;
		if ( identical(p, Three.RGBFormat) ) return WebGLRenderingContext.RGB;
		if ( identical(p, Three.RGBAFormat) ) return WebGLRenderingContext.RGBA;
		if ( identical(p, Three.LuminanceFormat) ) return WebGLRenderingContext.LUMINANCE;
		if ( identical(p, Three.LuminanceAlphaFormat) ) return WebGLRenderingContext.LUMINANCE_ALPHA;

		if ( identical(p, Three.AddEquation) ) return WebGLRenderingContext.FUNC_ADD;
		if ( identical(p, Three.SubtractEquation) ) return WebGLRenderingContext.FUNC_SUBTRACT;
		if ( identical(p, Three.ReverseSubtractEquation) ) return WebGLRenderingContext.FUNC_REVERSE_SUBTRACT;

		if ( identical(p, Three.ZeroFactor) ) return WebGLRenderingContext.ZERO;
		if ( identical(p, Three.OneFactor) ) return WebGLRenderingContext.ONE;
		if ( identical(p, Three.SrcColorFactor) ) return WebGLRenderingContext.SRC_COLOR;
		if ( identical(p, Three.OneMinusSrcColorFactor) ) return WebGLRenderingContext.ONE_MINUS_SRC_COLOR;
		if ( identical(p, Three.SrcAlphaFactor) ) return WebGLRenderingContext.SRC_ALPHA;
		if ( identical(p, Three.OneMinusSrcAlphaFactor) ) return WebGLRenderingContext.ONE_MINUS_SRC_ALPHA;
		if ( identical(p, Three.DstAlphaFactor) ) return WebGLRenderingContext.DST_ALPHA;
		if ( identical(p, Three.OneMinusDstAlphaFactor) ) return WebGLRenderingContext.ONE_MINUS_DST_ALPHA;

		if ( identical(p, Three.DstColorFactor) ) return WebGLRenderingContext.DST_COLOR;
		if ( identical(p, Three.OneMinusDstColorFactor) ) return WebGLRenderingContext.ONE_MINUS_DST_COLOR;
		if ( identical(p, Three.SrcAlphaSaturateFactor) ) return WebGLRenderingContext.SRC_ALPHA_SATURATE;
