...
HouseEntrance.prototype.onCreate = function(res) {
	...
	var dep = ResourceDepot.getInstance();
	this.arrow = this.isLeftEntrance ? dep.getImage("objective_arrows", "arrow_mirrored", true) : dep.getImage("objective_arrows", "arrow", true);
};
...
HouseEntrance.prototype.onInteract = function(obj, param) {
	...
	for (var i = 0; i < houses.length; i++) {
		...
		if (h.getId() == this.houseId) {
			status = h.getStatus();
			if (h.lit)
				app.audio.playDelayedFX(ResourceDepot.getInstance().getSFX("fire_crackle"), 500, true);
			break;
		}
	}
...
ShrapnelExploder.rattleOffsets = [new Vec2(3, -1), new Vec2(3, -2), new Vec2(2, -3), new Vec2(1, -2), new Vec2(-1, -1), new Vec2(-2, -1), new Vec2(-1, -2), new Vec2(1, -1), new Vec2(2, -1), new Vec2(1, 0), new Vec2(0, 0)];
...
ShrapnelExploder.prototype.draw = function(render, x, y) {
	if (!this.done) {
		...
		var xPos = (this.hitFromLeft ? ShrapnelExploder.rattleOffsets[ri].x : -ShrapnelExploder.rattleOffsets[ri].x) + x + (this.base ? this.base.getPos().x : 0);
		var yPos = ShrapnelExploder.rattleOffsets[ri].y + y + (this.base ? this.base.getPos().y : 0);

		if (!this.exploding && this.callback != null) {
			this.callback.draw(render, x + (this.base ? this.base.getPos().x : 0), y + (this.base ? this.base.getPos().y : 0), this.hitFromLeft ? ShrapnelExploder.rattleOffsets[ri].x : -ShrapnelExploder.rattleOffsets[ri].x, ShrapnelExploder.rattleOffsets[ri].y);
...
ShrapnelExploder.prototype.getRattleOffset = function() {
	var ri = Math.floor(this.rattleIndex);
	return new Vec2(this.hitFromLeft ? ShrapnelExploder.rattleOffsets[ri].x : -ShrapnelExploder.rattleOffsets[ri].x, ShrapnelExploder.rattleOffsets[ri].y);
};
