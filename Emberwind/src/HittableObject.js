...
HittableGameObject.prototype.draw = function(render, x, y) {
	if (this.displayHitEffect && !this.hitEffect.isStopped()) {
		this.hitEffect.draw(render, x + this.hitEffectOffset.x, y + this.hitEffectOffset.y, 0, 1, null);
	}
	if (this.blockEffect && !this.blockEffect.isStopped()) {
		this.blockEffect.draw(render, x + this.hitEffectOffset.x, y + this.hitEffectOffset.y, 0, 1, null);
	}
};
...
HittableGameObject.prototype.getHitEffectOffset = function(left) {
	return new Vec2( left ? -20 : 20, -20).add(new Vec2(Math.floor(randomRange(-5, 6)), Math.floor(randomRange(-5, 6))));
};
