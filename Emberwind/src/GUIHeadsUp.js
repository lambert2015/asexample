...
GUIHeadsUp.prototype.init = function() {
	...
	this.chainHitCounter = null;
	//todo: = new ScaleCounter("ChainHitCounterFont", 2, 4);

	this.chainHitText = null;
	//todo: = new ScaleCounter("ChainHitTextFont", 2, 4);
...
GUIHeadsUp.prototype.draw = function(render) {
	...
	if (this.chainHitCounter && this.chainHitTime > 0 && this.chainHits > 1) {
		this.chainHitText.draw(render, (x1 - 20 - this.chainHitText.getRect().width), (y0 + 20), this.getChainHitFadeFactor(), this.getChainHitFactor());
		this.chainHitCounter.draw(render, (x1 - 20 - (this.chainHitText.getRect().width + this.chainHitCounter.getRect().width) / 2), y0 + 10 + 30, this.getChainHitFadeFactor(), this.getChainHitFactor());
	}
...
function GUIHealthBar(left, gold) {
	...
	//this.idol[EmotionType.kEmoteNormal][IdolType.kIdolSiegeTower] =
	// dep.getImage("SiegeTowerIcon", "tower_icon");

	//this.idol[EmotionType.kEmoteNormal][IdolType.kIdolMortar] =
	// dep.getImage("MortarIcon", "default");
...
GUIHealthBar.prototype.draw = function(render, x, y) {
	if (!this.visible || this.idolEntries.length == 0)
		return;
...
function IdolEntry() {
	//IdolEntry() : metalType(kMetalGold), gemType(kGemYellow),
	// healthType(kGemYellow), idolType(IdolType.kIdolKindle),
	// emoteType(EmotionType.kEmoteNormal), timeLeft(0), priority(0), showHP(false) {
	// }
...
GUITextBubble.prototype.open = function(txt, title, timeTC) {
	...
	this.frame.setInteriorSize((r.width > titleRect.width ? r.width : titleRect.width) - this.safeX, r.height - this.safeY);
...
GUITransitionClock.prototype.getClockFan = function(width, height, start, end) {
	...
	var corners = [[width, 0, 1, 0], [0, 0, 0, 0], [0, height, 0, 1], [width, height, 1, 1]];
...
GUIObjectiveDisplay.prototype.countCompleted = function() {
	...
	if (this.numObjectives == this.numCompleted && false)
		app.game.dialogueTrigger("objective");
	// todo
};
...
GUIAcornDisplay.rattleOffsets = [new Vec2(3, -1), new Vec2(3, -2), new Vec2(2, -3), new Vec2(1, -2), new Vec2(-1, -1), new Vec2(-2, -1), new Vec2(-1, -2), new Vec2(1, -1), new Vec2(2, -1), new Vec2(1, 0), new Vec2(0, 0)];
	...
	switch (this.mode) {
		...
		case GUIAcornDisplay.Modes.kMultiplier:
			if (this.multiplierFadeTime < GUIAcornDisplay.kMulFadeTime * 0.2) {
				r.drawImage(this.acorn, c - Math.floor(this.acorn.width / 2) + GUIAcornDisplay.rattleOffsets[this.rattleIndex].x, 35 + GUIAcornDisplay.rattleOffsets[this.rattleIndex].y, 0, false, 1 - alpha);
		...
		default:
			r.drawImage(this.acorn, c - Math.floor(this.acorn.width / 2) + GUIAcornDisplay.rattleOffsets[this.rattleIndex].x, 35 + GUIAcornDisplay.rattleOffsets[this.rattleIndex].y, 0);
