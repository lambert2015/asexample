...
Game.prototype.loadStage = function(stageName, callback) {
	...
	this.hud.init();
	this.scoreObservers = [this.hud.scoreDisp, this.hud.acornDisp];
	// TEMP
...
Game.prototype.preloadCallback = function(status) {
	...
	if (this.externalPreloadCallback !== null)
		this.externalPreloadCallback(status);
};
...
Game.prototype.draw = function() {
	if (this.render.frontToBack) {
		if (this.fadeParam != 0)
			this.render.drawFillScreen(new Pixel32(0, 0, 0, this.fadeParam * 255));
...
Game.prototype.getScreenRect = function() {
	return new Rectf(-this.camera.x, -this.camera.y, -this.camera.x + app.maxWidth, -this.camera.y + app.maxHeight);
};
...
Game.prototype.setCurrentFoe = function(foe) {
	...
	if (this.currentFoe != foe) {
		if (this.currentFoe !== null)
			this.currentFoe.removeObserver(this.hud.healthBarRight);
		this.currentFoe = foe;
		if (foe !== null) {
...
Game.prototype.blastRect = function(r, hits, hitPlayer, burn, countAsChain) {
	...
	for (var i = 0; i < objects.length; i++) {
		...
		if (!( hittable instanceof HittableGameObject))
			continue;
		if (!( hittable instanceof Gremlin) && !( hittable instanceof PlayerCharacter))
			continue;
