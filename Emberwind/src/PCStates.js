...
IdleState.prototype.transition = function() {
	...
	return this.fsm.tryChangeState(!this.host.movement.IsOnFloor(), PCStates.kStateFall) || this.fsm.tryChangeState(this.fromSprint && (this.host.facingRight && input.held(Buttons.right) || !this.host.facingRight && input.held(Buttons.left)), PCStates.kStateSprint) || this.fsm.tryChangeState((input.doubleTapped(Buttons.left) || input.doubleTapped(Buttons.right)), PCStates.kStateSprint) || this.fsm.tryChangeState(input.held(Buttons.left) ^ input.held(Buttons.right), PCStates.kStateWalk) || this.fsm.tryChangeState(input.held(Buttons.up) && host.isNearLadder() && !host.movement.IsNearFloor(40, host.movement.CollisionMode.kDiscardNormal), PCStates.kStateClimb) || this.fsm.tryChangeState(input.held(Buttons.jump), PCStates.kStateJump) || this.fsm.tryChangeState(input.held(Buttons.down) && host.isNearLadder() && !host.movement.IsNearFloor(40, host.movement.CollisionMode.kDiscardOneSided), PCStates.kStateClimb) || this.fsm.tryChangeState(input.held(Buttons.down), PCStates.kStateHide) || this.fsm.tryChangeState(input.held(Buttons.attack) && this.btnHasBeenReleased, PCStates.kStateSwing);
};
...
WalkState.prototype.update = function(dt) {
	...
	// Calculate any speed boost that should be applied if we're getting close to
	// auto dash
	var speedBoost = 0;
	// TODO Game parameters

	if (this.host.facingRight) {
		this.host.movement.SetDesiredVelocityTan(150 + speedBoost);
		// TODO game param instead of 150
	} else {
		this.host.movement.SetDesiredVelocityTan(-150 - speedBoost);
		// TODO game param instead of 150
	}
...
WalkState.prototype.transition = function() {
	var input = GameInput.instance;
	return this.fsm.tryChangeState(!this.host.movement.IsOnFloor() && !this.host.movement.IsNearFloor(20) && this.timeInAir > 0.25, PCStates.kStateFall) || this.fsm.tryChangeState((input.doubleTapped(Buttons.left) || input.doubleTapped(Buttons.right)) || !this.noAutoSprint && this.timeInState > 1.2, PCStates.kStateSprint) ||
	// // TODO Game parameters
	this.fsm.tryChangeState(!(input.held(Buttons.left) ^ input.held(Buttons.right)), PCStates.kStateIdle) || this.fsm.tryChangeState(input.held(Buttons.jump), PCStates.kStateJump) || this.fsm.tryChangeState(input.held(Buttons.attack) && this.btnHasBeenReleased, PCStates.kStateSwing);
...
JumpState.prototype.enter = function(msg, fromState) {
	...
	if (msg === PCMessages.kTransitionedFromSprint) {
		this.host.movement.SetDesiredVelocityVert(-559);
		// TODO game parameters
		this.cameFromSprint = true;
	} else {
		this.host.movement.SetDesiredVelocityVert(-430);
		// TODO game parameters
		this.cameFromSprint = false;
	}
...
JumpState.prototype.transition = function() {
	var input = GameInput.instance;
	return this.fsm.tryChangeState(this.host.movement.IsOnFloor() && this.host.movement.desiredVel.y > 0, PCStates.kStateIdle, this.cameFromSprint ? PCMessages.kTransitionedFromSprint : -1) || this.fsm.tryChangeState(input.held(Buttons.attack) && this.btnHasBeenReleased, PCStates.kStateJumpAttack) || this.fsm.tryChangeState(this.host.movement.velocity.y > 0, PCStates.kStateFall, this.cameFromSprint ? PCMessages.kTransitionedFromSprint : -1);
...
FallState.prototype.transition = function() {
	...
	return this.fsm.tryChangeState(host.movement.IsOnFloor(), PCStates.kStateIdle, this.cameFromSprint ? PCMessages.kTransitionedFromSprint : -1) || this.fsm.tryChangeState(input.held(Buttons.attack) && this.btnHasBeenReleased, PCStates.kStateJumpAttack) || this.fsm.tryChangeState(this.host.isNearLadder() && input.held(Buttons.up) && !host.movement.IsNearFloor(40, this.host.movement.CollisionMode.kDiscardNormal), PCStates.kStateClimb);
};
...
ClimbState.prototype.transition = function() {
	var input = GameInput.instance;
	return this.fsm.tryChangeState(!this.host.isNearLadder(), PCStates.kStateFall) || this.fsm.tryChangeState((input.held(Buttons.left) || input.held(Buttons.right)) && !(input.held(Buttons.up) || input.held(Buttons.down)), PCStates.kStateFall) || this.fsm.tryChangeState(input.held(Buttons.down) && this.host.movement.IsNearFloor(40, this.host.movement.CollisionMode.kDiscardOneSided), PCStates.kStateIdle) || this.fsm.tryChangeState(input.held(Buttons.up) && this.host.movement.IsNearFloor(40, this.host.movement.CollisionMode.kDiscardNormal), PCStates.kStateIdle);
};
...
SprintState.prototype.update = function(dt) {
	...
	if (this.host.facingRight) {
		this.host.movement.SetDesiredVelocityTan(300);
		// TODO Game parameters
	} else {
		this.host.movement.SetDesiredVelocityTan(-300);
		// TODO Game parameters
...
SprintState.prototype.transition = function() {
	var input = GameInput.instance;
	return this.fsm.tryChangeState(!this.host.movement.IsOnFloor() && !this.host.movement.IsNearFloor(40) && this.timeInAir > 0.25, PCStates.kStateFall, PCMessages.kTransitionedFromSprint) || this.fsm.tryChangeState(this.host.facingRight && !input.held(Buttons.right) || !this.host.facingRight && !input.held(Buttons.left), PCStates.kStateIdle) || this.fsm.tryChangeState(input.held(Buttons.jump), PCStates.kStateJump, PCMessages.kTransitionedFromSprint) || this.fsm.tryChangeState(input.held(Buttons.attack), PCStates.kStateBarrelRollStart);
};
...
HideState.prototype.transition = function() {
	...
	var shuffle = (input.held(Buttons.left) || input.held(Buttons.right)) && ((this.host.facingRight && (input.held(Buttons.left))) || (!this.host.facingRight && (input.held(Buttons.right))) || (!this.host.movement.IsFacingEdge(!this.host.facingRight) && !this.host.movement.IsFacingWall(!this.host.facingRight)));

	// todo: rocket jump

	return this.fsm.tryChangeState(input.held(Buttons.up), PCStates.kStateUnhide) || this.fsm.tryChangeState(shuffle, PCStates.kStateStartShuffle);
};
...
SwingState.prototype.transition = function() {
	var input = GameInput.instance;
	return this.fsm.tryChangeState(!this.hammer && (input.held(Buttons.left) && input.held(Buttons.right)), PCStates.kStateWalk) || this.fsm.tryChangeState(this.animStopped && (!this.attackPressed || this.hammer), PCStates.kStateIdle);
};
...
BarrelRollState.prototype.transition = function() {
	...
	return this.fsm.tryChangeState(upHeld, PCStates.kStateJump) || this.fsm.tryChangeState(this.startedAsRightRoll && !rightHeld || !this.startedAsRightRoll && !leftHeld || downHeld, PCStates.kStateBarrelRollEnd);
};
...
ShuffleState.prototype.transition = function() {
	var input = GameInput.instance;
	var endShuf = !(input.held(Buttons.left) || input.held(Buttons.right)) || this.host.movement.IsFacingEdge(!this.host.facingRight) || this.host.movement.IsFacingWall(!this.host.facingRight);

	return this.fsm.tryChangeState(this.blasted, PCStates.kStateSnuffExplode) || this.fsm.tryChangeState(this.host.facingRight && (input.held(Buttons.left)), PCStates.kStateTurnShuffle) || this.fsm.tryChangeState(!this.host.facingRight && (input.held(Buttons.right)), PCStates.kStateTurnShuffle) || this.fsm.tryChangeState(endShuf, PCStates.kStateEndShuffle);
};
...
StartShuffleState.prototype.transition = function() {
	var input = GameInput.instance;
	return this.fsm.tryChangeState(this.blasted, PCStates.kStateSnuffExplode) || this.fsm.tryChangeState(this.animStopped && !(input.held(Buttons.left) || input.held(Buttons.right)), PCStates.kStateEndShuffle) || this.fsm.tryChangeState(this.animStopped, PCStates.kStateShuffle);
};
...
EndShuffleState.prototype.transition = function() {
	return this.fsm.tryChangeState(this.blasted, PCStates.kStateSnuffExplode) || this.fsm.tryChangeState(this.animStopped, PCStates.kStateHide, PCMessages.kAlreadyHidden);
};
