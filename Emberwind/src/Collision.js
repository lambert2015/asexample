...
LineSegment2.prototype.GetProjectionParam = function(p) {
	var m = this.b.copy().sub(this.a);
	// m = b - a
	var proj = p.copy().sub(this.a);
	// p - a
	return m.Dot(proj) / m.Dot(m);
	// m.(p-a) / m.m
};
...
function GetCollisionShape(circle, movement, shape) {
	...
	// Handle during loops
	var contact;
	// Placeholder for eventual contact point
...
function GetCollisionSphere(circleA, movementA, circleB, movementB) {
	...
	if (IntersectRaySphere(new Ray2(circleA.c, movementOfCircleA.mul(1.1)), cB)) {
		...
		var rhs = new Vec2(movementB.x, movementB.y);
		// RHS of -
		rhs.mul(localCollisionTime);
		rhs.add(circleB.c);

		contactPoint.normal = new Vec2(movementA.x, movementA.y);
		// LHS of -
		...
		rhs = new Vec2(contactPoint.normal.x, contactPoint.normal.y);
		// RHS of outer +
		rhs.mul(circleB.r);

		contactPoint.point = new Vec2(movementB.x, movementB.y);
		// LHS of outer +
