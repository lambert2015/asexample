...
SoundPlayer.prototype.PlayFX = function(handle, looping, category) {
	...
	if (channel !== null) {
		...
		}, 1000 * duration + ( looping ? 0 : 200));
		// Wait a bit before stopping (there's time..)
		channel.timer = timer;
...
SoundPlayer.prototype.draw = function(render) {
	...
	render.drawFilledRect(Math.floor(render.getWidth() / 2) - w / 2, y0, Math.floor(render.getWidth() / 2) + w / 2, y0 + h, new Pixel32(0, 0, 0, 128));
	...
	for (var c = 0; c < this.channels.length; c++) {
		...
		render.drawLine(barX, lineY - 5, barX, lineY + 5, audio_elem.paused ? lineCol : playCol);
