...
ResourceDepot.prototype.loadImageset = function(setName, localized) {
	...
	if (localized)
		filename = filename.replace(/english/, this.language);
...
ResourceDepot.prototype.loadTile = function(index) {
	...
	return new Tile(new EMBImage(image, source[0], source[1], width, height, xOffset, yOffset, textureWidth, textureHeight), t.shapes, index - ts.startid, t.name);
};
...
ResourceDepot.prototype.loadAnimation = function(index) {
	...
	for (var j = 0; j < sources.length; j++) {
		...
		frames.push(new EMBImage(image, source[0], source[1], width, height, xOffset, yOffset, textureWidth, textureHeight));
	}
