...
TutorialCard.prototype.doDraw = function(render, x, y) {
	...
	render.drawFilledRect(backRect.x0, backRect.y0, backRect.x1, backRect.y1, this.advanced ? TutorialTip.colorsAdvanced[0] : TutorialTip.colorsBasic[0], this.advanced ? TutorialTip.colorsAdvanced[1] : TutorialTip.colorsBasic[1]);
	...
	if (this.isLocked) {
		render.drawFilledRect(backRect.x0, backRect.y0, backRect.x1, backRect.y1, TutorialTip.colorsLocked[0], TutorialTip.colorsLocked[1]);
...
TutorialCardWalk.prototype.draw = function(render, x, y) {
	...
	render.drawImage(dpadRImage, x - 90, y - 40, 0, true, 1, new Pixel32(255, 255, 255, this.timeInState >= 0.5 && this.timeInState <= 2 ? 128 : 0));
};
...
function TutorialCardJumpClimb() {
	...
	this.tiles = [[-1, -1, -1, 4], [-1, -1, -1, 4], [85, 85, 95, 4], [-1, -1, -1, 4]];
	for (var x = 0; x < 4; ++x)
		for (var y = 0; y < 4; ++y) {
...
function TutorialCardEnter() {
	...
	this.indoorTiles = [[104, 104], [141, 151], [-1, 142], [-1, 142]];
	this.outdoorTiles = [[-1, 4], [-1, 4], [68, 78], [24, 27]];
	for (var x = 0; x < 4; ++x)
		for (var y = 0; y < 2; ++y) {
