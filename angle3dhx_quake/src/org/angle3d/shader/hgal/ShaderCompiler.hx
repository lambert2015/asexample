package org.angle3d.shader.hgal;
import flash.Lib;
...
class ShaderCompiler 
{
	 ...
	 * @param	shader
	 */
	public function complie(vertexSrc:String,fragmentSrc:String,shader:Shader):Void
	{
		_vertexData.clear();
		_lexer.parse(vertexSrc, _vertexData);
		
		_fragmentData.clear();
		_fragmentData.copyVaryingFrom(_vertexData);
		_lexer.parse(fragmentSrc, _fragmentData);
		
		shader.setVertexData(writeByteArray(_vertexData));
		shader.setFragmentData(writeByteArray(_fragmentData));
		
		sendDatatoShader(_vertexData, shader);
		sendDatatoShader(_fragmentData, shader);
	...
	public function getAgal(shaderType:String):String
	{
	 ...
	 * @param	shader
	 */
	private function sendDatatoShader(data:CompileData,shader:Shader):Void
	{
	...
	private function maskBits( m : Array<String> ):UInt
	{
		...
		for ( c in m )
		{
			bits |= 1 << getCharIndex(c);
		}
		return bits;
	}

	private function swizzleBits( s : Array<String> ):UInt
	{
		if ( s == null ) 
		   return 0 | (1 << 2) | (2 << 4) | (3 << 6); //0xe4
		   
		#if debug
		Assert.assert(s.length <= 4, "s的长度不能超过4");
		#end
