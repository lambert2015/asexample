...
class CurveAndSurfaceMath 
{
	...
	{
		Assert.assert(nurbSpline.getType() == SplineType.Nurb, "Given spline is not of a NURB type!"); 
	...
	public static function interpolate(u:Float, v:Float, 
	        controlPoints:Vector<Vector<Vector4f>>, 
			knots:Vector<Vector<Float>>, 
			basisUFunctionDegree:Int, 
			basisVFunctionDegree:Int, 
			store:Vector3f):Void 
	{
		store.setTo(0, 0, 0);
		var delimeter:Float = 0;
	...
	private static function computeBaseFunctionValue( i:Int, k:Int, t:Float, knots:Vector<Float>):Float 
	{
		...
		{
			return (t - knots[i]) / (knots[i + k - 1] - knots[i]) * 
					CurveAndSurfaceMath.computeBaseFunctionValue(i, k - 1, t, knots)
					+ (knots[i + k] - t) / (knots[i + k] - knots[i + 1]) * 
     ...
     */
    public static function getCatmullRomP1toP2Length(p0:Vector3f, 
	                            p1:Vector3f, 
	                            p2:Vector3f, 
	                            p3:Vector3f, 
	                            startRange:Float, 
	                            endRange:Float, 
	                            curveTension:Float):Float
	{
