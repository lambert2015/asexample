...
class BlendMode
{
	private static var sBlendFactors:Array<Dynamic> = [ 
	...
	{
		var modes:Dynamic = sBlendFactors[premultipliedAlpha ? 1 : 0];
		if (mode in modes) 
			return modes[mode];
		else 
									...
									premultipliedAlpha:Bool = true):Void
	{
		...
		var otherModes:Dynamic = sBlendFactors[premultipliedAlpha ? 1 : 0];
