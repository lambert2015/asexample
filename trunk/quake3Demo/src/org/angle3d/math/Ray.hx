...
class Ray implements Collidable
{
	...
	public function intersects(v0:Vector3f, v1:Vector3f, v2:Vector3f, 
	                            result:Vector3f,doPlanar:Bool,quad:Bool):Bool
	{
	...
	public function intersects2(v0:Vector3f, v1:Vector3f, v2:Vector3f):Float
	{
        ...
        var dirDotDiffxEdge2:Float = sign * (direction.x * diffEdge2X
        ...
        if (dirDotDiffxEdge2 >= 0.0) 
		{
            ...
            var dirDotEdge1xDiff:Float = sign * (direction.x * diffEdge2X
                                          +  direction.y * diffEdge2Y
                                          +  direction.z * diffEdge2Z);
	...
	public function collideWith(other:Collidable, results:CollisionResults):Int
	{
		...
		{
			var bv:BoundingVolume = Lib.as(other, BoundingVolume);
			return bv.collideWith(this, results);
		}
		else if (Std.is(other, AbstractTriangle))
		{
			var tri:AbstractTriangle = Lib.as(other, AbstractTriangle);
