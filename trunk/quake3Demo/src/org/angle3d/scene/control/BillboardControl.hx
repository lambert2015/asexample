...
class BillboardControl extends AbstractControl
{
	...
	{
		look.copyFrom(camera.getLocation());
    ...
    private function rotateScreenAligned(camera:Camera3D):Void
	{
        ...
        var parent:Node = spatial.getParent();
        var rot:Quaternion = new Quaternion();
		rot.fromRotationMatrix(orient);
        if ( parent != null ) 
		{
    ...
    private function rotateAxial(camera:Camera3D, axis:Vector3f):Void
	{
        ...
        look.copyFrom(camera.getLocation()).subtractLocal(spatial.getWorldTranslation());   
        var rotation:Quaternion = spatial.getParent().getWorldRotation();
