package com.adobe.utils {
	...
	public class AGALMiniAssembler {
		...
		protected static const COMPONENTS : Object = {120: 0, // x
				121: 1, // y
				122: 2, // z
				119: 3, // w
		...
		public function AGALMiniAssembler(debug : Boolean = false) : void {
			this.debug = debug;
			if (!initialized)
				init();
		}
		...
		public function assemble(mode : String, source : String) : ByteArray {
			...
			if (mode == FRAGMENT)
				...
				throw new Error('ERROR: mode needs to be "' + FRAGMENT + '" or "' + VERTEX + '" but is "' + mode + '".');
			...
			for (i = 0; i < lng; i++) {
				...
				if (opFound.flags & OP_DEC_NEST) {
					nest--;
					if (nest < 0) {
						throw new Error("error: conditional closes without open.");
						break;
					}
				}
				if (opFound.flags & OP_INC_NEST) {
					nest++;
					if (nest > MAX_NESTING) {
						throw new Error("error: nesting to deep, maximum allowed is " + MAX_NESTING + ".");
						break;
					}
				}
				if ((opFound.flags & OP_FRAG_ONLY) && !isFrag) {
					throw new Error("error: opcode is only allowed in fragment programs.");
					break;
				}
				if (verbose)
				...
				if (nops > MAX_OPCODES) {
					throw new Error("error: too many opcodes. maximum is " + MAX_OPCODES + ".");
					break;
				}
				...
				if (regs.length != opFound.numRegister) {
					throw new Error("error: wrong number of operands. found " + regs.length + " but expected " + opFound.numRegister + ".");
					break;
				}
				...
				for (var j : int = 0; j < regLength; j++) {
					...
					if (regFound == null) {
						throw new Error("error: could not parse operand " + j + " (" + regs[j] + ").");
						badreg = true;
						break;
					}
					...
					if (isFrag) {
						if (!(regFound.flags & REG_FRAG)) {
							throw new Error("error: register operand " + j + " (" + regs[j] + ") only allowed in vertex programs.");
							badreg = true;
							break;
						}
						if (isRelative) {
							throw new Error("error: register operand " + j + " (" + regs[j] + ") relative adressing not allowed in fragment programs.");
							badreg = true;
							break;
						}
					} else {
						if (!(regFound.flags & REG_VERT)) {
							throw new Error("error: register operand " + j + " (" + regs[j] + ") only allowed in fragment programs.");
							badreg = true;
							break;
						}
					}
					...
					if (regFound.range < regidx) {
						throw new Error("error: register operand " + j + " (" + regs[j] + ") index exceeds limit of " + (regFound.range + 1) + ".");
						badreg = true;
						break;
					}
					...
					if (isDest && isRelative) {
						throw new Error("error: relative can not be destination");
						badreg = true;
						break;
					}
					...
					if (isRelative) {
						...
						if (regFoundRel == null) {
							throw new Error("error: bad index register");
							badreg = true;
							break;
						}
