...
class RenderManager 
{
	...
	private var forcedMaterial:Material;
     ...
     * to return to normal
     */
	public function setForcedRenderState(forcedRenderState:RenderState):Void
	{
     ...
     */
	public function renderGeometry(g:Geometry):Void
	{
	...
	private function renderShadow(s:Spatial, rq:RenderQueue):Void
	{
		...
		else if (Std.is(s, Geometry))
		{
			...
			var shadowMode:ShadowMode = s.getShadowMode();
	...
	public function renderScene(scene:Spatial, vp:ViewPort):Void
	{
		...
		else if (Std.is(scene, Geometry))
		{
			...
			var shadowMode:ShadowMode = gm.getShadowMode();
	...
	public function renderViewPortQueues(vp:ViewPort, flush:Bool):Void
	{
		...
		{
			renderer.setDepthTest(false, Context3DCompareMode.ALWAYS);
            queue.renderQueue(QueueBucket.Sky, this, cam, flush);
        }

		renderer.setDepthTest(true, Context3DCompareMode.LESS_EQUAL);
		...
		{
			renderer.setDepthTest(false, Context3DCompareMode.LESS_EQUAL);
            queue.renderQueue(QueueBucket.Transparent, this, cam, flush);
        }

        if (!queue.isQueueEmpty(QueueBucket.Gui)) 
		{
			renderer.setDepthTest(true, Context3DCompareMode.ALWAYS);
            setCamera(cam, true);
	...
	public function renderViewPort(vp:ViewPort, tpf:Float):Void
	{
		...
		renderer.setFrameBuffer(vp.frameBuffer);
