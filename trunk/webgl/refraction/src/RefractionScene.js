...
// 4. In the refraction shader, refract only the pixels with their alpha channel
// set.
...
function RefractionScene() {
	...
	// Brightness shader for performing the final brightness, contrast, and gamma
	// correction
	...
	// viewport
	// back to its original size after rendering to the FBO, which may use a
	// different
	// dimension.
...
RefractionScene.prototype.OnMouseMove = function(key) {
	...
	if (owner.mMouseDown) {
		...
		owner.mViewMatrix.PointAt(new Point(Math.sin(owner.mCameraRot) * owner.mCameraZoom, owner.mCameraHeight, Math.cos(owner.mCameraRot) * owner.mCameraZoom), new Point());
...
RefractionScene.prototype.Start = function() {
	...
	// Brightness shader for adjusting the brightness, contrast, and gamma correction
	// for the final
	// rendered image.
...
RefractionScene.prototype.Update = function() {
	...
	if (this.mLoadComplete) {
		...
		//
		// Step 2: Draw geometry affected by refraction. This will not be visible in the
		// rendered image,
		//		   but it will alter the alpha channel of the fragment, which is used as an
		// indicator to the
...
RefractionScene.prototype.LoadShaders = function(index) {
	if (index == 0) {
		...
		// Create shader program to tag pixels that will be affected by the refraction
		// shader
		...
		this.mPixelTagShader.AddShader(this.mShader[0]);
		// basic.vs
		this.mPixelTagShader.AddShader(this.mShader[2]);
		// pixel_tag.fs
		...
		this.mRefractionShader.AddShader(this.mShader[3]);
		// image.vs
		this.mRefractionShader.AddShader(this.mShader[4]);
		// refraction.fs
		...
		this.mSkyboxShader.AddShader(this.mShader[5]);
		// skybox.vs
		this.mSkyboxShader.AddShader(this.mShader[6]);
		// skybox.fs
		...
		this.mBrightnessShader.AddShader(this.mShader[3]);
		// image.vs
		this.mBrightnessShader.AddShader(this.mShader[7]);
		// brightness.fs
...
RefractionScene.prototype.OnLoadComplete = function() {
	...
	this.mViewMatrix.PointAt(new Point(0.0, this.mCameraHeight, this.mCameraZoom), new Point());
