...
/**
 * 每次最大下载次数有限制,最多同时下载MAX_COUNT个文件
 * @author andy
 */
class TextureManager 
{
	private static var MAX_COUNT:Int = 3;
	...
	private var _textureMap:HashMap<String,TextureBase>;
	...
	public function addTexture(bitmapData:BitmapData,name:String):Void
	{
		...
		Logger.log("addTexture:"+name);
		
		//TODO 检查name是否已经存在，若存在了，则警告
		_textureMap.setValue(name, t);
	}
	...
	{
		var t:TextureBase = _textureMap.getValue(name);
		if (t == null)
		{
