...
class Matrix4f
{
	public static inline var IDENTITY:Matrix4f = new Matrix4f();
     ...
     *            the matrix to copy.
     */
	public inline function copyFrom(mat:Matrix4f):Void
	{
		this.m00 = mat.m00;
		this.m01 = mat.m01;
		this.m02 = mat.m02;
     ...
     * 
     * @param result
     *            the list to set the values into.
     * @param rowMajor
     *            whether the outgoing data is in row or column major order.
     */
	public function toUniform(result:Vector<Float> = null, rowMajor:Bool = true):Vector<Float>
	{
		if (result == null)
		{
     ...
     * @return this
     */
	public inline function setColumn(column:Int, vector:Vector4f):Void
	{
		#if debug
		Assert.assert(column >= 0 && column <= 3, "Invalid column index.");
		#end
     ...
     * @return this
     */
	//public inline function setVector2(matrix:Vector<Vector<Float>>):Void
	//{
		//#if debug
		//Assert.assert(matrix.length == 4 && matrix[0].length == 4, "matrix & matrix[0].length must be of size 4.");
		//#end
		//
		//for (i in 0...4)
		//{
	...
	//}
	//
	//public inline function setArray2(matrix:Array<Array<Float>>):Void
	//{
		//#if debug
		//Assert.assert(matrix.length == 4 && matrix[0].length == 4, "matrix & matrix[0].length must be of size 4.");
		//#end
	...
	public function fromFrustum(near:Float, far:Float, left:Float, right:Float, 
	                            top:Float, bottom:Float, parallel:Bool = false):Void 
	{
        loadIdentity();
     ...
     *            the scale to apply
     */
    public function scaleByVec(scale:Vector3f):Void
	{
		var sx = scale.x;
		var sy = scale.y;
		var sz = scale.z;
     ...
     * @return the rotated vector.
     */
	public inline function multVecAcross(vec:Vector3f, ?result:Vector3f=null):Vector3f
	{
		if (result == null)
		{
			result = new Vector3f();
     ...
     * @return the rotated vector.
     */
	public inline function multNormal(vec:Vector3f, result:Vector3f = null):Vector3f
	{
		if (result == null)
		{
			result = new Vector3f();
	...
	public inline function multNormalAcross(vec:Vector3f, ?result:Vector3f=null):Vector3f
	{
		if (result == null)
		{
			result = new Vector3f();
     ...
     * @return the W value
     */
	public inline function multProj(vec:Vector3f, result:Vector3f):Float
	{
		var vx:Float = vec.x, vy:Float = vec.y, vz:Float = vec.z;
        result.x = m00 * vx + m01 * vy + m02 * vz + m03;
        result.y = m10 * vx + m11 * vy + m12 * vz + m13;
     ...
     * @return the rotated vector.
     */
	public inline function multVec4(vec:Vector4f, ?result:Vector4f=null):Vector4f
	{
		if (result == null)
		{
			result = new Vector4f();
     ...
     * @return the rotated vector.
     */
	public inline function multVec4Across(vec:Vector4f, ?result:Vector4f=null):Vector4f
	{
		if (result == null)
		{
			result = new Vector4f();
     ...
     */
	public inline function multQuat(vec:Quaternion, ?result:Quaternion=null):Quaternion
	{
		if (result == null)
		{
	...
	public inline function multLocalQuat(rotation:Quaternion):Void
	{
	...
	public function getRotationQuat(result:Quaternion = null):Quaternion
	{
		if (result == null)
		{
			result = new Quaternion();
		}
		result.fromRotationMatrix(getRotationMatrix());
		return result;
	}
	
	public function getRotationMatrix(mat:Matrix3f = null):Matrix3f
	{
		if (mat == null)
		{
			mat = new Matrix3f();
     ...
     *             if translation is not size 3.
     */
	public function setInverseTranslation(trans:Vector3f):Void
	{
		m03 = -trans.x;
		m13 = -trans.y;
		m23 = -trans.z;
	...
	public function angleRotation(angles:Vector3f):Void
	{
        ...
        angle = (angles.z * FastMath.DEGTORAD);
        sy = Math.sin(angle);
        cy = Math.cos(angle);
        angle = (angles.y * FastMath.DEGTORAD);
        sp = Math.sin(angle);
        cp = Math.cos(angle);
        angle = (angles.x * FastMath.DEGTORAD);
        sr = Math.sin(angle);
        cr = Math.cos(angle);
     ...
     */
	public function setInverseRotationRadians(angles:Vector3f):Void
	{
	...
	public function setInverseRotationDegrees(angles:Vector3f):Void
	{
		var vec:Vector3f = angles.clone();
		vec.x = (angles.x * FastMath.RADTODEG);
        vec.y = (angles.y * FastMath.RADTODEG);
        vec.z = (angles.z * FastMath.RADTODEG);
     ...
     */
    public function inverseTranslateVect(vec:Vector3f):Void
	{
        vec.x -= m03;
        vec.y -= m13;
        vec.z -= m23;
