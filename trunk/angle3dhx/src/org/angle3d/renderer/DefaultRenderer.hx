...
class DefaultRenderer implements IRenderer
{
	...
	public function setShader(shader:Shader):Void
	{
		...
		var program:Program3D = ShaderManager.instance.getProgram(_shader.name, _context3D);
		...
		//上传Shader数据
		_shader.uploadShader(_context3D);
	}
	...
	public inline function setVertexAttrib(mesh:Mesh):Void
	{
		...
		for (i in 0...buffersList.length)
		{
			...
			{
				_shader.setAttribute(_context3D, "a_" + vb.getType(), vb3d);
			}
		}
	}
}