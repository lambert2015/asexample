...
//
	...
	public function setSphereByFourPoints(D:Vector3f, A:Vector3f, B:Vector3f, C:Vector3f):Void
	{
		...
		var Denominator:Float = 2.0 * (a.x * (b.y * c.z - c.y * b.z) - b.x
	...
	{
		radius = Math.sqrt(((A.x - D.x) * (A.x - D.x) + (A.y - D.y)
                * (A.y - D.y) + (A.z - D.z) * (A.z - D.z)) / 4) + RADIUS_EPSILON - 1;
				
		D.interpolate(A, 0.5, center);
	}
	...
	override public function transform(trans:Transform,?result:BoundingVolume=null):BoundingVolume
	{
		...
		if (result == null || result.getType() != BoundingVolumeType.Sphere)
		{
			sphere = new BoundingSphere();
		}
		else
		{
			sphere = Lib.as(result, BoundingSphere);
		}
	...
	private function getMaxAxis(scale:Vector3f):Float {
        var x:Float = FastMath.fabs(scale.x);
        var y:Float = FastMath.fabs(scale.y);
        var z:Float = FastMath.fabs(scale.z);
	...
	override public function clone(?result:BoundingVolume=null):BoundingVolume
	{
		...
		sphere = Lib.as(super.clone(sphere), BoundingSphere);
	...
	{
		if (FastMath.fabs(bb.center.x - center.x) < radius + bb.xExtent &&
            FastMath.fabs(bb.center.y - center.y) < radius + bb.yExtent &&
            FastMath.fabs(bb.center.z - center.z) < radius + bb.zExtent)
            return true;
