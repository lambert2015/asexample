...
class Intersection 
{
	...
	private static inline function findMinMax(x0:Float, x1:Float, x2:Float, minMax:Vector3f)
	{
        minMax.setTo(x0, x0, 0);
        if (x1 < minMax.x) minMax.x = x1;
        if (x1 > minMax.y) minMax.y = x1;
        if (x2 < minMax.x) minMax.x = x2;
        if (x2 > minMax.y) minMax.y = x2;
    }
	
	public static function intersect(bbox:BoundingBox,
	                                v1:Vector3f,v2:Vector3f,v3:Vector3f):Bool
	{
       ...
       var fex:Float = FastMath.fabs(e0.x);
       var fey:Float = FastMath.fabs(e0.y);
       var fez:Float = FastMath.fabs(e0.z);
