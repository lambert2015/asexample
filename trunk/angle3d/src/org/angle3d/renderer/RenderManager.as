package org.angle3d.renderer
{
	...
	public class RenderManager
	{
		...
		private var _forcedMaterial:Material;
		...
		public function renderGeometry(geom:Geometry):void
		{
			...
			var mat:Material;
			if (this._forcedMaterial != null)
			{
				mat = this._forcedMaterial;
			}
			else
			{
			...
			var shader:Shader;
			var length:int = techniques.length;
			for (var i:int = 0; i < length; i++)
			{
		...
		public function renderScene(scene:Spatial, vp:ViewPort):void
		{
			...
			if (!scene.checkCulling(vp.camera))
			{
				...
				if ((scene.shadowMode != ShadowMode.Off ||
					(scene is Node)) && scene.cullHint != CullHint.Always)
				{
