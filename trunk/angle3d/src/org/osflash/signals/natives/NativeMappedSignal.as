	   ...
	   * relayed from an IEventDispatcher, into other forms of data,
		...
		override public function set valueClasses(value:Array):void
		{
			...
			for (var i:int=_valueClasses.length; i--; )
			{
				...
				{
					throw new ArgumentError('Invalid valueClasses argument: ' + 'item at index ' + i + ' should be a Class but was:<' + _valueClasses[i] + '>.' + getQualifiedClassName(_valueClasses[i]));
				}
			}
		}
		...
		public function mapTo(... objectListOrFunction):NativeMappedSignal
		{
			if (objectListOrFunction.length == 1 && objectListOrFunction[0] is Function)
			{
				...
				{
					throw new ArgumentError('Mapping function has ' + _mappingFunction.length + ' arguments but it needs zero or one of type Event');
				}
			}
			else
			{
		...
		override public function dispatchEvent(event:Event):Boolean
		{
			...
			{
				throw new ArgumentError('Mapping returned ' + getQualifiedClassName(mappedData) + ', expected ' + valueClasses[0] + '.');
			}
