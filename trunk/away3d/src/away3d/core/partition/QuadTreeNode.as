package away3d.core.partition
{
	...
	public class QuadTreeNode extends NodeBase
	{
		...
		private function findPartitionForBounds(entityWorldBounds:Vector.<Number>):QuadTreeNode
		{
			...
			{
				if (left)
					return _leftNear.findPartitionForBounds(entityWorldBounds);
				else
					return _rightNear.findPartitionForBounds(entityWorldBounds);
			}
			else
			{
				if (left)
					return _leftFar.findPartitionForBounds(entityWorldBounds);
				else
					return _rightFar.findPartitionForBounds(entityWorldBounds);
			}
		}
	}
}
