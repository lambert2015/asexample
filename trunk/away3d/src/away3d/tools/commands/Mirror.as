	...
	public class Mirror
	{
		...
		*/
		...
		private static function getNextAvailableSubGeometry(index:uint, vectors:Array, materials:Vector.<MaterialBase>):int
		{
			...
			for (i; i < len; i += 5)
				if (mat == materials[i / 5] && Vector.<uint>(vectors[i + 1]).length < LIMIT)
					return i;
					...
					if (duplicate)
					{
						...
						if (destverts.length == LIMIT)
						{
							...
							if (i != materials.length - 1)
								nextID = getNextAvailableSubGeometry(i, vectors, materials);
