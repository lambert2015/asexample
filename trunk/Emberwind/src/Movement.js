...
function BaseMovementComponent() {
	...
	this.gravityStack = [];
	// @type {[number]}
	this.collisionMode = this.CollisionMode.kNormal;
	// @type CollisionMode
...
BaseMovementComponent.prototype.ClipToContacts = function(v, bouncyness) {
	bouncyness = typeof (bouncyness) != 'undefined' ? bouncyness : 0;
	// Default value
...
BaseMovementComponent.prototype.IsFacingWall = function(left) {
	...
	if (res.foundCollision && res.contactTime <= 1) {
		if ((res.contact.normal.Dot(new Vec2( left ? 1 : -1, 0)) > kWallAngle) && (res.contact.normal.Dot(this.velocity) <= 0)) {
			return true;
		}
	}
...
BaseMovementComponent.prototype.IsFacingEdge = function(left) {
	var tmpPos = this.position.copy().add(new Vec2( left ? -this.spheres[0].r * 2 : this.spheres[0].r, 0));
...
MovementComponent.prototype.Update = function(dt) {
	var _vec2buf;
	// Buffer during vector arithmetic

	var oldPosition = new Vec2(this.position.x, this.position.y);

	// Guard against unhandelable infinities, no one will miss a frame rendered at
	// this speed anyway
	...
	// We pretend that the impulse is a force of exactly the right size, hence the
	// divide by 'dt'
	...
	// Clip remainder of velocity to resting contacts, this might avoid oscilations
	// (MovementComponent.cpp:67)
