...
Fireplace.prototype.update = function(dt) {
	...
	if (this.numBlasts > 0) {
		var game = app.game;
		game.blastRect(game.currentStage.gamePlayLayerExtent, -1, false, true); --this.numBlasts;
	}
};
...
Fireplace.prototype.light = function() {
	if (this.interactVolume !== null) {
		...
		this.removeTriggers();
		game.sendMessageToId(this.houseId, {
			"houseStatus" : true
		});

		game.sendMessageToType(Villager, {
			"houseStatusNOW" : true
		});
...
Fireplace.prototype.spawn = function(bounce) {
	...
	if (!this.lit && this.numSpawned < this.spawnCount) {++this.numSpawned;
