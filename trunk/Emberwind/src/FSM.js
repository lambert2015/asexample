...
FSM.prototype.setState = function(state, msg, suspendCurrent) {
	if (state == fsmStates.kNextState)
		state = this.currentState + 1;
	assert(state >= fsmStates.kNoState && state < this.numStates, "Trying to set invalid state " + state + ".");
	assert(state == fsmStates.kNoState || this.stateArray[state].stateId == state, "Trying to set already active state.");
	...
	if (this.currentState != fsmStates.kNoState) {
		if (this.stateArray[this.currentState].isSuspended) {
			assert(this.currentState == this.suspendedArray[this.numSuspended - 1], "Not resuming the most recently suspended state!");
			this.stateArray[this.currentState].resume(msg, lastState);
			this.stateArray[this.currentState].isSuspended = false; --this.numSuspended;
		} else {
...
AppFSM.prototype.onMouse = function(x, y, left, leftPressed) {
	var s = this.getCurrentState();
	if (s)
		s.onMouse(x, y, left, leftPressed);
};
