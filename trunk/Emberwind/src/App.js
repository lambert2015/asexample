...
function App(canvasId, shaders, initCallback) {
	...
	var statesList = [appStates.kStateIntro, IntroState, appStates.kStateDeveloperSplash, DeveloperSplashState, appStates.kStateMainMenu, MainMenuState, appStates.kStateLoadingStage, LoadingStageState, appStates.kStateInGame, InGameState, appStates.kStateStageComplete, StageCompleteState, appStates.kStateYesNoDialog, YesNoDialogState, appStates.kStateTutorialPage, TutorialPageState, appStates.kStateDialogue, DialogueState, appStates.kStoryBoardState, StoryboardState];
	...
	this.screenTransitionsDefaultOut = new ScreenTransitionFade(0.25, new Pixel32(0, 0, 0, 0), new Pixel32(0, 0, 0, 255));
...
App.prototype.tryChangeState = function(condition, toState, msg, reEnter, suspendedCurrent) {
	if (reEnter === undefined)
		reEnter = true;
	if (suspendedCurrent === undefined)
		suspendedCurrent = false;
	if (toState == fsmStates.kNextState)
		toState = this.fsm.currentState + 1;
...
App.prototype.setState = function(state, msg, suspendCurrent) {
	...
	if (suspendCurrent || state == -1 || this.fsm.isSuspended(state)) {
		...
		if (st == null)
			st = this.findScreenTransition(this.fsm.currentState, -1, true);
		if (st == null)
			st = this.screenTransitionsDefaultOut;
	...
	if (this.stateMode == this.stateModes.kStateActive) {
		...
		if (this.screenTransition.isDone) {
			if (this.stateMode == this.stateModes.kTransitionOut) {
				...
				if (st == null)
					st = this.findScreenTransition(-1, this.newState, false);
				if (st == null)
					st = this.screenTransitionsDefaultIn;
...
App.prototype.draw = function() {
	if (this.render.frontToBack) {
		...
		if (this.screenTransition)
			this.screenTransition.draw(this.render);
		if (this.showFPS)
			this.drawFPS();
		if (this.debug && GameInput.instance)
			this.drawDebug();
		if (this.showSkipButton)
			this.drawSkipButton();
...
App.prototype.drawDebug = function() {
	...
	if (!this.useTouch)
		str = "mouse: " + inp.mouse.x + " " + inp.mouse.y + " " + str;
	this.render.drawSystemText(str, 10, 10, this.render.white);

	if (this.frontToBack)
		this.render.drawSystemText("Front to back", 730, 10, this.render.white);

	if (this.debugAudio)
		this.audio.draw(this.render);
};
...
App.prototype.drawFPS = function() {
	...
	if (this.frameCount == 10) {
		...
		if (this.frameDate != null) {
			...
			this.frameText = this.frameFont.generateCanvas("FPS: " + frameRate + " : " + (this.freezeFrame ? this.render.maxCalls + " / " : "") + this.render.calls);
		}
		this.frameDate = date;
		this.frameCount = 0;
	}
	if (this.frameText != null)
		this.render.drawText(this.frameText, 630, 15, 1, true);
};
...
App.prototype.drawSkipButton = function() {
	...
	if (!this.useTouch)
		this.render.drawImage(this.getControlImage(Buttons.skip), 750, 565, 0, true);
};
...
App.prototype.createRender = function(useGL) {
	...
	if (this.freezeFrame)
		this.render.maxCalls = oldRender.maxCalls;
