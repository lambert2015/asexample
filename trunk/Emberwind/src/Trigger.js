 ...
 * If the volume has a target the events are sent to it. If not the events are
 * sent to the entity entering.
 *
 * @param name is string
 * @param rect is a Rectf or null
 * @param parent is a TriggerParticle or null
 * @param target is an object with onTrigger(event, volume, object) method, or
 * null
...
/**
 * Set is a collection where each element is unique, elements are compared by
 * reference.
 ...
 * @param property is optional string, means that we compare to a field of set
 * elements
 * @param isContainer is optional boolean, if property is absent then isContainer
 * is true.
...
Set.prototype.findIndex = function(element, property, isContainer) {
	...
	assert(isProperty || isContainer);
	// !isProperty -> isContainer
...
/**
 * TriggerSystemObject captures relation between trigger object and trigger
 * system volumes
 *
 * @param object is a TriggerObject
...
/**
 * TriggerSystemVolume captures relation between trigger volume and trigger
 * system objects
 *
 * @param volume is a TriggerVolume
 * @param flush is an initial state, defaults to false
...
/**
 * Implicit grid is a spatial partitioning class. The world is partitioned into a
 * uniform
 * grid and the objects inserted are stored in row and column bit arrays. To
 * check if an
 * object with id 'i' exists in a specific grid cell you check bit 'i' in the row
 * and column
 * bit arrays.
 * This representation makes it very easy to check for potential overlaps by
 * using some
 * bit arithmetic.
 *
...
/**
 * Try to initialize the grid with the given parameters. If there's not enough
 * memory
 * the grid will reduce its resolution to fit. If there's not enough memory to
 * make
...
TriggerSystem.prototype.addObject = function(object) {
	assert( object instanceof TriggerObject);
	if (this.objectDeathRow.remove(object.particle) == null || this.objects.find(object, "object", false) == null) {
...
TriggerSystem.prototype.addVolume = function(volume) {
	// insert whenever volume is not on death row and is not within volumes
	if (this.volumeDeathRow.remove(volume) == null || this.volumes.find(volume, "volume", false) == null) {
...
TriggerSystem.prototype.update = function(ext) {
	...
	if (this.grid.init(ext.width, ext.height, ext.width / (96 * 2), ext.height / (96 * 2), this.volumes.size())) {
		...
		for ( i = 0; i < this.objects.size(); i++) {
			...
			// there's any volume that were culled away completely. If we find such a volume
			// we need
