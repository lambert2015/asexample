...
Stage.prototype.initStage = function(preloadCallback) {
	...
	this.gameplayExtent = {
		w : this.gamePlayLayerExtent.width,
		h : this.gamePlayLayerExtent.height
	};
 ...
 * @param {Function} preloadCallback callback that will run when the progress
 * updates.
		...
		for (var j = 0; j < objects.length; j++) {
			...
			switch (o.type) {
				...
				case "lamppost":
					...
					// Place it first so it doesn't conceal any other object.
					continue;
				case "objectgroup":
...
Stage.prototype.drawBackgrounds = function(frontToBack, render, x, y) {
	if (frontToBack)
		this.particleSystem.draw(render, x, y, false);
	for (var i = 0; i < this.backgrounds.length; i++) {
...
Stage.prototype.drawForegrounds = function(frontToBack, render, x, y) {
	if (frontToBack)
		this.particleSystem.draw(render, x, y, true);
