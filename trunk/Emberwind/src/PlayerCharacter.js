...
function PlayerCharacter() {
	...
	var statesList = [PCStates.kStateIdle, IdleState, PCStates.kStateWalk, WalkState, PCStates.kStateJump, JumpState, PCStates.kStateFall, FallState, PCStates.kStateClimb, ClimbState, PCStates.kStateSprint, SprintState, PCStates.kStateHide, HideState, PCStates.kStateUnhide, UnhideState, PCStates.kStateSwing, SwingState, PCStates.kStateDamaged, DamagedState, PCStates.kStateKnockedOut, KnockedOutState, PCStates.kStateJumpAttack, JumpAttackState, PCStates.kStateDialogueEvent, DialogueEventState, PCStates.kStateBarrelRollStart, BarrelRollStartState, PCStates.kStateBarrelRoll, BarrelRollState, PCStates.kStateBarrelRollEnd, BarrelRollEndState, PCStates.kStateStartShuffle, StartShuffleState, PCStates.kStateShuffle, ShuffleState, PCStates.kStateEndShuffle, EndShuffleState, PCStates.kStateTurnShuffle, TurnShuffleState];
...
PlayerCharacter.prototype.update = function(dt) {
	if (this.isAlive() && GameInput.instance.pressed(Buttons.interact)) {//todo:
		// check if not dialogue state
		this.useFireflies();
...
PlayerCharacter.prototype.playEffectAnim = function(anim, worldSpace) {
	if (worldSpace === undefined)
		worldSpace = true;

	var effect = new this.Effect( worldSpace ? this.EffectType.kWorldSpace : this.EffectType.kLocal, worldSpace ? this.movement.position : new Vec2(0, 0), anim);
...
PlayerCharacter.prototype.onAnimationEvent = function(evt, anim) {
	switch (evt) {
		case "stopped" :
			this.fsm.message(PCMessages.kAnimStopped);
			break;
		case "start_axe_attack" :
			this.startAttack(PCAttackType.kHammerAttack);
			break;
		case "start_swipe_attack" :
			this.startAttack(PCAttackType.kNormalAttack);
			break;
		case "start_jump_attack" :
			this.startAttack(PCAttackType.kJumpAttack);
			break;
		case "end_attack" :
			this.endAttack();
			break;
		case "step-in" :
			...
			break;
		case "step_sound" :
			app.audio.playFX(this.walkSound);
			break;
		case "shuffle_sound" :
			app.audio.playFX(this.waddleSound);
			break;
	}
...
PlayerCharacter.prototype.enterCave = function(targetStage, targetEntrance, houseId, status) {
	if (houseId != -1) {
		app.game.sendMessageToType(Fireplace, {
			houseId : houseId
		});
	}
};
...
PlayerCharacter.prototype.handleBlast = function() {
	...
	var doHandle = (state == PCStates.kStateHide) || (state == PCStates.kStateUnhide) || (state == PCStates.kStateStartShuffle) || (state == PCStates.kStateShuffle) || (state == PCStates.kStateEndShuffle) || (state == PCStates.kStateTurnShuffle);

	if (doHandle)
		this.fsm.message(PCMessages.kBlasted);
	return doHandle;
};
