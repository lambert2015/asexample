...
RenderCanvas.prototype.drawImage = function(image, x, y, angle, centered, alpha, tint, hFlipped) {
	...
	if (!this.frontToBack)
		this.drawEMBImage(image, x, y, angle !== 0, this.context);
	...
	if (tint != null && tint.a != 0) {
		...
		if (this.secondCanvas.width != image.width)
			this.secondCanvas.width = image.width;
		if (this.secondCanvas.height != image.height)
			this.secondCanvas.height = image.height;
...
RenderCanvas.prototype.drawEMBImage = function(img, x, y, angled, context) {
	...
	if (this.scaleFactor !== 1 && !angled) {
		var xs = this.scale(x);
		var ys = this.scale(y);
		context.drawImage(img.image, img.x, img.y, img.width, img.height, xs - x, ys - y, this.scale(x + img.width - xs), this.scale(y + img.height - ys));
	} else {
...
RenderCanvas.prototype.drawParticle = function(image, x, y, angle, scaleX, scaleY, alpha, color, additive) {
	if (this.calls++ > this.maxCalls)
		return;
	if (color != null)
		assert(color.r == 255 && color.b == 255 && color.g == 255, "Special draw particle");
	this.context.save();
	this.context.translate(x, y);
