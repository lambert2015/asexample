...
GremIdleState.prototype.transition = function() {
	...
	var condition = (this.host.type == Gremlin.types.kGremStandard || this.host.type == Gremlin.types.kGremGuard || this.host.type == Gremlin.types.kGremScout || this.host.type == Gremlin.types.kGremWarrior ) && this.host.combatAwarenessTarget != null && this.attackDelay <= 0;

	return this.fsm.tryChangeState(condition, Gremlin.states.kStateSwing) || this.fsm.tryChangeState(this.host.isInPursuit(), Gremlin.states.kStatePursue);
};
...
GremSwingState.prototype.enter = function(msg, fromState) {
	var right = this.host.combatAwarenessTarget ? (this.host.combatAwarenessTarget.getPos().x - this.host.getPos().x > 0) : false;
...
GremWalkState.prototype.enter = function(msg, fromState) {
	this.speedMultiplier = (this.host.type == Gremlin.types.kGremThief || this.host.type == Gremlin.types.kGremWarrior || this.host.type == Gremlin.types.kGremBouncer) ? 1.5 : 1;
...
GremWalkState.prototype.update = function(dt) {
	...
	if (this.leaderUpdate <= 0 && this.host.type == Gremlin.types.kGremBouncer && this.host.hasLeader()) {
		...
		this.host.walkTarget = (new Vec2((!this.host.leader.isFacingRight() ? this.walkOffset : -this.walkOffset), 0)).add(this.host.leader.getPos());
		this.turnToTarget();
	}
...
GremWalkState.prototype.transition = function() {
	...
	if (this.timeToLootCheck <= 0 && this.host.type == Gremlin.types.kGremThief) {
		...
		return true;
	} else if ((this.host.type == Gremlin.types.kGremStandard || this.host.type == Gremlin.types.kGremScout || this.host.type == Gremlin.types.kGremBouncer) && this.host.combatAwarenessTarget && this.minTimeToAttack <= 0) {
		this.fsm.setState(Gremlin.states.kStateSwing);
		return true;
...
GremJumpState.prototype.transition = function(message) {
	return this.fsm.tryChangeState(this.host.movement.IsOnFloor() && this.host.movement.GetDesiredVelocity().y > 0, this.host.getDefaultState()) || this.fsm.tryChangeState(this.host.movement.velocity.y > 0, Gremlin.states.kStateFall);
};
...
GremDamagedState.prototype.transition = function(message) {
	return this.fsm.tryChangeState(this.stopped && this.host.movement.IsOnFloor(), this.host.type == Gremlin.types.kGremWarrior ? Gremlin.states.kStateBlock : this.host.getDefaultState());
};
