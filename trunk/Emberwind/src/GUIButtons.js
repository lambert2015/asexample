...
GUIBox.prototype.update = function(dt) {
	...
	if (keyboard.pressed(Buttons.up)) {
		if (this.keyboardPosition == null)
			this.keyboardPosition = this.buttons.length - 1;
		else
			this.keyboardPosition = (this.keyboardPosition - 1 + this.buttons.length) % this.buttons.length;
	} else if (keyboard.pressed(Buttons.down)) {
		if (this.keyboardPosition == null)
			this.keyboardPosition = 0;
		else
			this.keyboardPosition = (this.keyboardPosition + 1) % this.buttons.length;
...
function GUITextButton(par, id, label, font, interiorRect, normalImg, highlightedImg, pressedImg, disabledImg, pressedTextoffset, callback) {
	if (pressedTextoffset == null)
		pressedTextoffset = new Point2(1, 1);
...
GUITextButton.prototype.states = {
	normal : 0,
	highlighted : 1,
	pressed : 2,
	disabled : 3
