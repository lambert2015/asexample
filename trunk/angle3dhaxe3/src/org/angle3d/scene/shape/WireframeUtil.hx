...
class WireframeUtil
{
	...
	public static function generateNormalLineShape(mesh:Mesh, size:Float = 5):WireframeShape
	{
		...
		{
			var subMesh:SubMesh = subMeshList[i];
			if (subMesh.getVertexBuffer(BufferType.POSITION) == null || 
				subMesh.getVertexBuffer(BufferType.NORMAL) == null)
			{
				continue;
			}

			var vertices:Vector<Float> = subMesh.getVertexBuffer(BufferType.POSITION).getData();
			var normals:Vector<Float> = subMesh.getVertexBuffer(BufferType.NORMAL).getData();
