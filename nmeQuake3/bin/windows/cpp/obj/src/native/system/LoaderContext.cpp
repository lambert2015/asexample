...
Void LoaderContext_obj::__construct(hx::Null< bool >  __o_checkPolicyFile,::native::system::ApplicationDomain applicationDomain,::native::system::SecurityDomain securityDomain)
{
HX_STACK_PUSH("LoaderContext::new","native/system/LoaderContext.hx",14);
bool checkPolicyFile = __o_checkPolicyFile.Default(false);
{
	...
	if (((applicationDomain != null()))){
		...
		HX_STACK_LINE(23)
		this->applicationDomain = ::native::system::ApplicationDomain_obj::currentDomain;
	}
}
;
	return null();
...
hx::ObjectPtr< LoaderContext_obj > LoaderContext_obj::__new(hx::Null< bool >  __o_checkPolicyFile,::native::system::ApplicationDomain applicationDomain,::native::system::SecurityDomain securityDomain)
...
Dynamic LoaderContext_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	switch(inName.length) {
	case 14:
		if (HX_FIELD_EQ(inName,"securityDomain") ) { securityDomain=inValue.Cast< ::native::system::SecurityDomain >(); return inValue; }
		break;
	case 15:
		...
		break;
	case 17:
		if (HX_FIELD_EQ(inName,"applicationDomain") ) { applicationDomain=inValue.Cast< ::native::system::ApplicationDomain >(); return inValue; }
		break;
	case 27:
...
{
	hx::Static(__mClass) = hx::RegisterClass(HX_CSTRING("native.system.LoaderContext"), hx::TCanCast< LoaderContext_obj> ,sStaticFields,sMemberFields,
