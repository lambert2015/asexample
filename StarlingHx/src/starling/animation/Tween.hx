...
class Tween extends EventDispatcher implements IAnimatable
{
	...
	private var mTransitionFunc:Dynamic;
	...
	public function reset(target:Dynamic, time:Float, transition:Dynamic="linear"):Tween
	{
		...
		else if (Type.typeof(transition) == TFunction)
		...
		if (mProperties != null)  
			...
			mProperties  = new Vector<String>();
			
		if (mStartValues != null) 
			mStartValues.length = 0; 
		else 
			mStartValues = new Vector<Float>();
			
		if (mEndValues != null)   
			mEndValues.length   = 0; 
		else 
			mEndValues   = new Vector<Float>();
	...
	public var transitionFunc(get, set):Dynamic;
	...
	private function get_transitionFunc():Dynamic 
	{ 
		return mTransitionFunc; 
	}
	private function set_transitionFunc(value:Dynamic):Void
	{
	...
	{
		return mRoundToInt = value; 
	}        
	
	/** A function that will be called when the tween starts (after a possible delay). */
	private function get_onStart():Dynamic 
	{ 
		return mOnStart; 
	}
	private function set_onStart(value:Dynamic):Dynamic 
	{ 
		return mOnStart = value; 
	}
	
	/** A function that will be called each time the tween is advanced. */
	private function get_onUpdate():Dynamic 
	{ 
		return mOnUpdate; 
	}
	private function set_onUpdate(value:Dynamic):Dynamic 
	{ 
		return mOnUpdate = value; 
	}
	
	/** A function that will be called each time the tween finishes one repetition
	 *  (except the last, which will trigger 'onComplete'). */
	private function get_onRepeat():Dynamic 
	{ 
		return mOnRepeat; 
	}
	private function set_onRepeat(value:Dynamic):Dynamic 
	{ 
		return mOnRepeat = value; 
	}
	
	/** A function that will be called when the tween is complete. */
	private function get_onComplete():Dynamic 
	{ 
		return mOnComplete; 
	}
	private function set_onComplete(value:Dynamic):Dynamic 
	{ 
		return mOnComplete = value; 
	}
