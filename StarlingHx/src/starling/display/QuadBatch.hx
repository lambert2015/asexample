...
/** Optimizes rendering of a Float of quads with an identical state.
 * 
...
class QuadBatch extends DisplayObject
{
	...
	private static var sRenderAlpha:Vector<Float> = Vector.ofArray<Float>([1.0, 1.0, 1.0, 1.0]);
										  ...
										  ignoreCurrentFilter:Bool=false):Int
	{
		...
		else if (container)
		{
			...
			for (i in 0...numChildren)
			{
				...
				var childVisible:Bool = child.alpha  != 0.0 && child.visible && 
		...
		if (isRootObject)
		{
			...
			var j:Int = quadBatches.length - 1;
			while (j > quadBatchID)
			{
