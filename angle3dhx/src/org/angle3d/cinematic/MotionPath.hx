...
import org.angle3d.scene.shape.CurveShape;
...
class MotionPath extends EventDispatcher
{
	...
	public function interpolatePath(time:Float, control:MotionTrack):Float
	{
        ...
        switch (spline.getType())
		{
            ...
            case SplineType.Linear:
                ...
                val = val / spline.getSegmentLengthAt(control.getCurrentWayPoint());
	...
	{
        var lg:WireframeGeometry = new WireframeGeometry("LinearPath", new CurveShape(spline, 0));
		Lib.as(lg.getMaterial(), MaterialWireframe).setColor(0x0000ff);
        return lg;
    }

    private function createCatmullRomPath():Geometry
	{
        var lg:WireframeGeometry = new WireframeGeometry("CatmullRomPath", new CurveShape(spline, 10));
		Lib.as(lg.getMaterial(), MaterialWireframe).setColor(0x0000ff);
        return lg;
    }
     ...
     */
	public function getSplineType():Int
	{
     ...
     */
	public function setSplineType(type:Int):Void
	{
		spline.setType(type);
	...
	private function refreshDebugNode():Void
	{
		if (debugNode != null) 
		{
            var parent:Node = debugNode.parent;
