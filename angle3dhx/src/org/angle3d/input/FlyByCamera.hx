...
class FlyByCamera implements AnalogListener,implements ActionListener
{
	...
	public function registerWithInput(inputManager:InputManager):Void
	{
        ...
        inputManager.addMapping("FLYCAM_Left", Vector.ofArray([new MouseAxisTrigger(MouseInput.AXIS_X, true),
                                               new KeyTrigger(Keyboard.LEFT)]));

        inputManager.addMapping("FLYCAM_Right", Vector.ofArray([new MouseAxisTrigger(MouseInput.AXIS_X, false),
                                                new KeyTrigger(Keyboard.RIGHT)]));

        inputManager.addMapping("FLYCAM_Up", Vector.ofArray([new MouseAxisTrigger(MouseInput.AXIS_Y, false),
                                             new KeyTrigger(Keyboard.UP)]));

        inputManager.addMapping("FLYCAM_Down", Vector.ofArray([new MouseAxisTrigger(MouseInput.AXIS_Y, true),
	...
	{
		// derive fovY value
        var h:Float = cam.getFrustumTop();
        var w:Float = cam.getFrustumRight();
        var aspect:Float = w / h;

        var near:Float = cam.getFrustumNear();
	...
	private function riseCamera(value:Float):Void
	{
        ...
        var pos:Vector3f = cam.getLocation().clone();
