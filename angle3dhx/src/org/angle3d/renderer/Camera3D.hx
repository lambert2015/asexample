...
class Camera3D 
{
    ...
    public function copyFrom(cam:Camera3D):Void
	{
        ...
        if (cam.projectionMatrixOverride != null) 
		{
			...
			{
        		projectionMatrixOverride = cam.projectionMatrixOverride.clone();
        	} 
			else 
			{
        		projectionMatrixOverride.copyFrom(cam.projectionMatrixOverride);
        	}
        } 
		else 
		{
     ...
     */
	public function setClipPlane(clipPlane:Plane, side:Int):Void
	{
	...
	public function contains(bound:BoundingVolume):Int
	{
		...
		while (planeCounter-- > 0)
		{
			...
			{
				var side:Int = bound.whichSide(worldPlanes[planeId]);
