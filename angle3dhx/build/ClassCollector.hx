...
class ClassCollector
{
	...
	public static function main():Void
	{
		...
		if (!neko.FileSystem.exists(outputDir))
		{
			Lib.println("creating output directory: " + outputDir);
			neko.FileSystem.createDirectory(outputDir);
		}
		...
		try
		{
			...
			if (genClass)
			{
				for (i in output)
					s += "import " + i + ";\n";
				s += "class " + out.substr(0, out.length - 3) + "{}"; //remove .hx
			}
			else
			if (genProperty)
			{
