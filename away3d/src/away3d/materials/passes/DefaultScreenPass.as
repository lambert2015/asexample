...
//		private var _commonsData : Vector.<Number> = Vector.<Number>([.5, 0, 0, 1]);
		...
		override arcane function activate(stage3DProxy:Stage3DProxy, camera:Camera3D, textureRatioX:Number, textureRatioY:Number):void
		{
			...
			if (_normalDependencies > 0 && _normalMethod.hasOutput)
				_normalMethod.activate(_normalMethodVO, stage3DProxy);
			_ambientMethod.activate(_ambientMethodVO, stage3DProxy);
			if (_shadowMethod)
				_shadowMethod.activate(_shadowMethodVO, stage3DProxy);
			_diffuseMethod.activate(_diffuseMethodVO, stage3DProxy);
			if (_usingSpecularMethod)
				_specularMethod.activate(_specularMethodVO, stage3DProxy);
			if (_colorTransformMethod)
				_colorTransformMethod.activate(_colorTransformMethodVO, stage3DProxy);
		...
		arcane override function deactivate(stage3DProxy:Stage3DProxy):void
		{
			...
			if (_normalDependencies > 0 && _normalMethod.hasOutput)
				_normalMethod.deactivate(_normalMethodVO, stage3DProxy);
			_ambientMethod.deactivate(_ambientMethodVO, stage3DProxy);
			if (_shadowMethod)
				_shadowMethod.deactivate(_shadowMethodVO, stage3DProxy);
			_diffuseMethod.deactivate(_diffuseMethodVO, stage3DProxy);
			if (_usingSpecularMethod)
				_specularMethod.deactivate(_specularMethodVO, stage3DProxy);
			if (_colorTransformMethod)
				_colorTransformMethod.deactivate(_colorTransformMethodVO, stage3DProxy);
		...
		arcane override function render(renderable:IRenderable, stage3DProxy:Stage3DProxy, camera:Camera3D, lightPicker:LightPickerBase):void
		{
			...
			var context:Context3D = stage3DProxy._context3D;
			if (_uvBufferIndex >= 0)
				stage3DProxy.setSimpleVertexBuffer(_uvBufferIndex, renderable.getUVBuffer(stage3DProxy), Context3DVertexBufferFormat.FLOAT_2, renderable.UVBufferOffset);
			if (_secondaryUVBufferIndex >= 0)
				stage3DProxy.setSimpleVertexBuffer(_secondaryUVBufferIndex, renderable.getSecondaryUVBuffer(stage3DProxy), Context3DVertexBufferFormat.FLOAT_2, renderable.secondaryUVBufferOffset);
			if (_normalBufferIndex >= 0)
				stage3DProxy.setSimpleVertexBuffer(_normalBufferIndex, renderable.getVertexNormalBuffer(stage3DProxy), Context3DVertexBufferFormat.FLOAT_3, renderable.normalBufferOffset);
			if (_tangentBufferIndex >= 0)
				stage3DProxy.setSimpleVertexBuffer(_tangentBufferIndex, renderable.getVertexTangentBuffer(stage3DProxy), Context3DVertexBufferFormat.FLOAT_3, renderable.tangentBufferOffset);
			...
			if (_shadowMethod)
				_shadowMethod.setRenderState(_shadowMethodVO, renderable, stage3DProxy, camera);
			_diffuseMethod.setRenderState(_diffuseMethodVO, renderable, stage3DProxy, camera);
			if (_usingSpecularMethod)
				_specularMethod.setRenderState(_specularMethodVO, renderable, stage3DProxy, camera);
			if (_colorTransformMethod)
				_colorTransformMethod.setRenderState(_colorTransformMethodVO, renderable, stage3DProxy, camera);
		...
		arcane override function invalidateShaderProgram(updateMaterial:Boolean = true):void
		{
			...
			if (_shadowMethod)
				addPasses(_shadowMethod.passes);
			addPasses(_diffuseMethod.passes);
			if (_specularMethod)
				addPasses(_specularMethod.passes);
			if (_colorTransformMethod)
				addPasses(_colorTransformMethod.passes);
		...
		{
			if (_normalMethod)
				_normalMethod.initConstants(_normalMethodVO);
			if (_diffuseMethod)
				_diffuseMethod.initConstants(_diffuseMethodVO);
			if (_ambientMethod)
				_ambientMethod.initConstants(_ambientMethodVO);
			if (_specularMethod)
				_specularMethod.initConstants(_specularMethodVO);
			if (_shadowMethod)
				_shadowMethod.initConstants(_shadowMethodVO);
			if (_colorTransformMethod)
				_colorTransformMethod.initConstants(_colorTransformMethodVO);
		...
		private function cleanUpMethods():void
		{
			...
			if (_ambientMethod)
				_ambientMethod.cleanCompilationData();
			if (_specularMethod)
				_specularMethod.cleanCompilationData();
			if (_shadowMethod)
				_shadowMethod.cleanCompilationData();
			if (_colorTransformMethod)
				_colorTransformMethod.cleanCompilationData();
		...
		private function compile():void
		{
			...
			if (_projectionDependencies > 0)
				compileProjCode();
			if (_uvDependencies > 0)
				compileUVCode();
			if (_secondaryUVDependencies > 0)
				compileSecondaryUVCode();
			if (_globalPosDependencies > 0)
			...
			if (_shadowMethod)
				updateMethodRegisters(_shadowMethod);
			updateMethodRegisters(_ambientMethod);
			if (_specularMethod)
				updateMethodRegisters(_specularMethod);
			if (_colorTransformMethod)
				updateMethodRegisters(_colorTransformMethod);
		...
		private function calculateDependencies():void
		{
			...
			if (_shadowMethod)
				setupAndCountMethodDependencies(_shadowMethod, _shadowMethodVO);
			setupAndCountMethodDependencies(_ambientMethod, _ambientMethodVO);
			if (_usingSpecularMethod)
				setupAndCountMethodDependencies(_specularMethod, _specularMethodVO);
			if (_colorTransformMethod)
				setupAndCountMethodDependencies(_colorTransformMethod, _colorTransformMethodVO);
...
//				_registerCache.removeVertexTempUsage(_globalPositionVertexReg);
		...
		private function compileTangentNormalMapFragmentCode():void
		{
			...
			if (_normalMethodVO.needsView)
				_registerCache.removeFragmentTempUsage(_viewDirFragmentReg);
			if (_normalMethodVO.needsGlobalPos)
				_registerCache.removeVertexTempUsage(_globalPositionVertexReg);
		...
		private function compileLightingCode():void
		{
			...
			if (_ambientMethodVO.needsNormals)
				_registerCache.removeFragmentTempUsage(_normalFragmentReg);
			if (_ambientMethodVO.needsView)
				_registerCache.removeFragmentTempUsage(_viewDirFragmentReg);
			...
			if (_diffuseMethodVO.needsNormals)
				_registerCache.removeFragmentTempUsage(_normalFragmentReg);
			if (_diffuseMethodVO.needsView)
				_registerCache.removeFragmentTempUsage(_viewDirFragmentReg);
			...
			if (_usingSpecularMethod)
			{
				...
				if (_specularMethodVO.needsNormals)
					_registerCache.removeFragmentTempUsage(_normalFragmentReg);
				if (_specularMethodVO.needsView)
					_registerCache.removeFragmentTempUsage(_viewDirFragmentReg);
			}
		}
		...
		private function initLightRegisters():void
		{
			...
			for (i = 0; i < len; ++i)
			{
				...
				if (_lightDataIndex == -1)
					_lightDataIndex = _dirLightRegisters[i].index * 4;
			}
			...
			for (i = 0; i < len; ++i)
			{
				...
				if (_lightDataIndex == -1)
					_lightDataIndex = _pointLightRegisters[i].index * 4;
			}
		}
		...
		private function compilePointLightCode():void
		{
			...
			for (var i:uint = 0; i < _numPointLights; ++i)
			{
				...
				if (_lightDataIndex == -1)
					_lightDataIndex = lightPosReg.index * 4;
