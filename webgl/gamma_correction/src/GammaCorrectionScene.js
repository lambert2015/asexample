...
// This scene demonstrates how to correct the gamma for a render. Your monitor
// applies a
// gamma curve to images displayed on the screen, but renderers operate in linear
// space.
// You must apply the inverse gamma to the rendered image before it is sent to
// the monitor.
...
function GammaCorrectionScene() {
	...
	// Shader for rendering 2D images onto the screen. Used in the texture compare
	// scene.
...
GammaCorrectionScene.prototype.OnGammaValueChanged = function(event, ui) {
	...
	// Adjust the gamma in the basic shader to "uncorrect" the texture. This is step
	// 3.
...
GammaCorrectionScene.prototype.Update = function() {
	...
	if (this.mLoadComplete) {
		// For the calibration screen, show a black background to make it easier to
		// distinguish
		// changes.
...
GammaCorrectionScene.prototype.LoadShaders = function(index) {
	if (index == 0) {
		...
		this.mImageShader.AddShader(this.mShader[2]);
		// image.vs
		this.mImageShader.AddShader(this.mShader[3]);
		// image.fs
		...
		this.mBrightnessShader.AddShader(this.mShader[2]);
		// image.vs
		this.mBrightnessShader.AddShader(this.mShader[4]);
		// brightness.fs
...
GammaCorrectionScene.prototype.OnLoadComplete = function() {
	...
	// Process textures
	this.mTexture.push(this.mResource.Find("brick.jpg"));
	// 24 bpp
	this.mTexture.push(this.mResource.Find("white.png"));
	// 8 bpp
	this.mTexture.push(this.mResource.Find("checkerboard.png"));
	// 8 bpp
	this.mTexture.push(this.mResource.Find("gradient_10.png"));
	// 8 bpp
	this.mTexture.push(this.mResource.Find("gradient_blacks.png"));
	// 8 bpp
	...
	// Assign the default "white" texture to all objects (necessary to avoid problems
	// in the basic fragment shader)
