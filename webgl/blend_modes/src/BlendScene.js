// This scene demonstrates the various blending operations by manually
// calculating the
// blending inside a shader. This is useful when you need to expand on OpenGL's
// minimal
...
BlendScene.prototype.start = function() {
	...
	// White texture can be tinted to different colours
	...
	// Sliders
	var sliders = ["#SrcRedSlider", this.onSrcRedSliderChanged, "#SrcGreenSlider", this.onSrcGreenSliderChanged, "#SrcBlueSlider", this.onSrcBlueSliderChanged, "#DstRedSlider", this.onDstRedSliderChanged, "#DstGreenSlider", this.onDstGreenSliderChanged, "#DstBlueSlider", this.onDstBlueSliderChanged];
	this.mColourSlider = [];
...
BlendScene.prototype.compileShaders = function(index, list, blendModes) {
	...
	if (shaderItem != null) {
		...
		if (!shader.create((shaderItem.name.lastIndexOf(".vs") != -1) ? GLShader.ShaderType.Vertex : GLShader.ShaderType.Fragment, shaderCode)) {
	...
	if (index < list.length) {
		...
		var parent = this;
		setTimeout(function() {
			parent.compileShaders(index, list, blendModes)
		}, 1);
...
BlendScene.prototype.loadShaders = function(index, blendModes) {
	...
	blendShader.create();
	blendShader.addShader(this.mShader[0]);
	// image.vs
	blendShader.addShader(this.mShader[index + 1]);
	// blend.fs
	blendShader.link();
	blendShader.init();
	...
	if (!this.mLoadComplete) {
		...
		var parent = this;
		setTimeout(function() {
			parent.loadShaders(index, blendModes)
		}, 1);
	}
}
// Implementation.
